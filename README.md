...
手机号是业务唯一身份。Web 端可使用手机号+密码或手机号+短信验证码登录；教育邮箱只用于校园认证，不作为登录凭证。小程序采用「微信登录 → 选择学校 → 输入手机号 → 阿里云短信验证码 → 绑定 OpenID 与手机号 → 登录」，无需传统注册，也不依赖 `getPhoneNumber` 授权弹窗。学校选择只用于新账号首次绑定；已有账号始终进入服务端已绑定学校，后续只能在个人主页切换学校。
...
| 本地 Mock 短信示例账号（江南） | `13800138000` | 未设置 | Mock 环境复用；可登录后设置密码 |

其中，已认证演示账号另外绑定教育邮箱；`13800138000` 为未认证、无密码的本地 Mock 示例账号。真实短信环境必须填写本人可接收验证码的手机号，验证码核验通过后后端才会创建或复用手机号账号并绑定当前微信 OpenID。
