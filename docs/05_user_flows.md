...
**涉及数据实体：** Post、Category、Location、Tag

---

### F02：新用户注册并选择学校

**触发条件：** 用户点击"注册"按钮，或尝试执行需要登录的操作（发布、点赞、收藏等）
...
3. 用户点击信息卡片
4. 系统跳转到信息详情页
5. 系统加载并展示完整信息：标题、分类、地点、描述、图片、标签、发布时间、有效期
6. 用户查看发布者信息（昵称、头像）
7. 用户查看有效性状态和确认人数
8. 用户查看互动数据（点赞数、收藏数、评论数）
9. 用户浏览评论区（默认显示最新 10 条）
10. 用户可进行点赞、收藏、评论、确认有效性等操作（需登录）
...
**涉及数据实体：** Post、Category、Location、Tag、User、Comment、Like、Favorite、ValidationRecord
...
- 地图加载失败：显示错误提示和重试按钮
- 位置授权拒绝：使用默认校园中心点作为地图中心，提示用户可手动拖动
...
11. 第六步：添加标签（选填，最多 5 个，可从系统预设标签选择或自定义输入）
12. 第七步：设置有效期（选填，可选永久、7天、30天、90天、自定义日期；系统根据分类给出默认建议）
13. 第八步：选择是否匿名发布
14. 用户点击"预览"按钮
15. 系统展示信息发布预览效果
16. 系统进行敏感信息检测（手机号、身份证号等），如有则提醒用户
17. 用户确认无误后点击"发布"按钮
18. 系统提交信息，如开启审核则状态为"待审核"，否则直接"已发布"
19. 系统跳转到信息详情页，显示发布成功提示
...
**涉及数据实体：** Post、Category、Location、Tag、PostImage
...
**触发条件：** 用户在信息详情页或信息卡片上点击"点赞"或"收藏"按钮

**前置条件：** 用户已登录

**主要步骤：**

1. 用户在信息详情页或信息卡片上看到"点赞"和"收藏"按钮
2. 用户点击"点赞"按钮
3. 系统发送点赞请求到后端
4. 系统更新点赞数并实时显示
5. 按钮变为已点赞状态（高亮）
...
1. 用户在个人中心点击"我的收藏"
2. 系统展示用户收藏的信息列表，按收藏时间倒序排列
3. 用户可点击信息进入详情页
4. 用户可在列表中直接取消收藏
5. 收藏的信息被删除时，自动从收藏列表中移除
6. 支持分页加载

**成功结果：** 用户成功查看和管理自己的发布与收藏
...
**涉及页面：** 个人中心页、我的发布页、我的收藏页、信息详情页

**涉及数据实体：** Post、Favorite、User
...
| F03 查看详情 | → F08 点赞收藏、F09 评论、F10/F11 确认有效性、F13 举报 |
| F04 地图发现 | → F03 查看详情 |
| F05 搜索 | → F03 查看详情 |
