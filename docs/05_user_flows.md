...
2. 输入本人手机号并获取 `login` 用途的阿里云短信验证码；同一手机号 60 秒内不能重复发送，验证码 5 分钟内有效。
3. 提交手机号和 6 位验证码后，后端先完成微信 `code2Session` 与短信核验，再按手机号复用或创建账号，并写入 `wechat_miniprogram` OpenID 身份。
4. 微信已经绑定其他真实手机号时拒绝静默换绑；首次创建的账号默认未认证、无密码，登录后可在个人中心认证教育邮箱和设置密码。
