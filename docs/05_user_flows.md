...
1. 点击“微信登录”，小程序调用 `wx.login` 获取临时 code，不请求微信手机号授权弹窗。
2. 选择学校，再输入本人手机号并获取 `login` 用途的阿里云短信验证码；同一手机号 60 秒内不能重复发送，验证码 5 分钟内有效。
3. 提交手机号和 6 位验证码后，后端先完成微信 `code2Session` 与短信核验，再按手机号复用或创建账号，并写入 `wechat_miniprogram` OpenID 身份。新账号同时创建所选学校的唯一 active membership。
4. 已有手机号账号不会因登录页选择而切换学校，客户端以响应中的 `user.school_id` 同步 `campusStore`，首页、地图和学校作用域请求随绑定学校变化。
5. 只有个人主页“切换学校”会调用 `POST /schools/{code}/join` 修改唯一 active membership 和 `user.school_id`；微信已经绑定其他真实手机号时仍拒绝静默换绑。首次创建的账号默认未认证、无密码。
