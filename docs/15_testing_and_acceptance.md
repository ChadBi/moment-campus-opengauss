...
| TC028 | 查看消息通知 | 1. 点击通知图标<br>2. 进入通知页 | 显示未读和已读通知列表 | P0 |
| TC029 | 标记通知已读 | 1. 点击通知项<br>2. 跳转到相关内容 | 通知标记为已读，未读数-1 | P0 |
| TC030 | 举报内容 | 1. 在信息详情页<br>2. 点击举报<br>3. 选择举报类型<br>4. 填写说明<br>5. 提交举报 | 举报成功，提示等待审核 | P0 |
| TC031 | 管理员审核-通过 | 1. 进入审核后台<br>2. 查看待审核信息<br>3. 点击通过 | 信息状态变为已发布 | P0 |
| TC032 | 管理员审核-拒绝 | 1. 进入审核后台<br>2. 查看待审核信息<br>3. 点击拒绝<br>4. 填写拒绝原因 | 信息状态变为已拒绝，通知发布者 | P0 |
| TC033 | 移动端响应式 | 1. 使用移动设备访问<br>2. 浏览各页面 | 布局正确，触摸操作正常 | P0 |
| TC034 | 游客操作限制 | 1. 未登录状态<br>2. 尝试点赞/评论/发布 | 提示需要登录，跳转到登录页 | P0 |
