...
> **历史评估（已废弃）**：本文早期将实时定位列为 P0。现行小程序不申请 `scope.userLocation`，不调用 `getLocation` 或 `chooseLocation`，帖子地点改为学校静态地点列表/地图选择；地点详情与 AI 摘要以《此刻校园：AI 地点摘要实施方案》为准。
...
5. Web、数据库与 API 已统一为 GCJ-02，并完成三校 39 个演示地点目录；小程序仍需在真机地图上抽查 `demo_approximate` 点位。
...
- 首期无需购买个性化地图样式。
- 数据库与 API 已统一使用 GCJ-02，小程序直接复用同一坐标契约。
...
| 协同验证/举报    | validations、report 接口                                         | 保留证实/证伪 2 类验证与独立举报语义                  |
...
可以新增轻量共享模块 `packages/contracts/`，只放 API 类型、Post 六态、证实/证伪两类协同验证、错误码和纯函数；不要把 React DOM 组件共享给小程序。
...
9. 普通用户评论、点赞，并分别完成 confirmation/refutation 的创建、切换与取消；更新建议、过期报告和冲突报告仅属于历史五类方案，不纳入现行验收。
...
| `demo_approximate` 点位与微信底图存在视觉差异 | P1   | 按三校点位目录做真机逐点抽查，发现偏差后回写目录与迁移          |
...
## 18. 2026-08 手机号主账号方案实施记录

本报告早期关于“邮箱登录/微信邮箱绑定”的设计已被当前方案替代：`users.id` 继续作为数据库物理主键，`users.phone` 作为业务唯一身份；小程序通过微信手机号授权调用 `POST /auth/wechat/phone-login`，Web 使用手机号短信或密码登录。教育邮箱仅用于校园认证，绑定关系为一对一，解绑需手机号短信确认。当前实现与验收结果详见 `AIwork/手机号主账号微信登录与教育邮箱认证改造_任务报告.md`。
