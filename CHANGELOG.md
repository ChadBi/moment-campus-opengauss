...
- **仓库整理里程碑版本**：统一长期滞后的版本号（README.md 由 2.0.0、frontend/package.json 由 2.1.0 同步至 2.3.0）；.gitignore 补加 `delete/` 规则（AGENTS.md 规约要求但此前缺失）；修正 AGENTS.md「根目录两个 HTML 文件」过时描述（已于 2026-07-26 归档至 `rubbish/`）；为严重过时的 `docs/18_项目现状说明.md`（描述"原始方案/Base 版本"与现状严重不符）添加废弃声明；清理全仓 `__pycache__`/孤儿 `.pyc` 及 `rubbish/` 内日志/截图/临时文件。

## [2.2.35] - 2026-08-09

### 修复

- **彻底修复跨校管理员登录需要两次点击问题**：登出时主动清除zustand持久化的学校上下文（`currentSchoolCode`），避免旧用户学校残留；新增"登录初始化"标记，登录后首次学校自动切换时静默完成不弹"无权限"误导性提示；支持已登录状态下直接换号登录场景（检测`user.id`变化）；修复退出登录后redirect参数指向`/login`导致登录后跳回登录页的死循环问题。三校管理员跨校登录均一次点击成功，无误导性提示。
...
- **修复跨校管理员登录需要两次点击问题**：登录成功后立即根据返回的`user.school_id`从公开学校列表找到对应学校并设置为当前学校，避免先跳转再检测到旧学校无权限后才切换，导致首次API请求失败和"您无当前学校的权限"提示，现在一次点击即可正确登录。

## [2.2.33] - 2026-08-09

### 修复

- **修复任务记录页面403权限错误**：「任务记录」是跨校平台级功能（帖子过期任务worker扫描所有学校），后端使用`SuperAdminDep`仅超管可访问，之前前端菜单未标记`superAdminOnly`，校管理员菜单中可见但访问返回403。现将「任务记录」移入平台运营菜单组，补充路由守卫反向拦截（校管理员访问平台路由自动重定向回管理首页）。

## [2.2.32] - 2026-08-09

### 修复

- **超管权限边界修复**：超管（`super_admin`）登录后默认跳转平台运营首页，仅显示「平台运营」菜单组（平台首页/套餐管理/学校管理/开通向导/激活漏斗/任务记录），不再显示校级管理菜单；新增路由守卫，超管访问校级页面自动重定向到平台首页，校管理员访问平台页面自动重定向到管理首页，权限隔离彻底。
- **小程序补充地点资料弹窗优化**：修复资料类型选择器显示英文key（normal_hours等）导致文本截断问题，改为显示中文标签；重构弹窗布局，为每个字段添加清晰标签，优化弹窗高度（max-height:85vh）、内边距、textarea最小高度和行高，彻底解决内容被挤压压缩问题。
- **Web管理后台地点资料审核完善**：后端审核队列接口改为Join查询，返回地点名称和提交人昵称；前端审核卡片显示地点名称、提交人、提交时间、资料内容（带标签）、补充说明；新增可折叠的驳回原因输入框；审核按钮固定宽度；支持分页；空状态友好提示。

## [2.2.31] - 2026-08-09

### 修复

- 修复小程序个人主页加载用户信息未经过 `normalizeUser` 规范化，导致 `avatar_url` 未经过 `resolveAvatar` 拼接 IMAGE_HOST 前缀，头像无法显示的问题。`loadUser` 改为调用已封装 `getMe()` 函数，统一走规范化处理。
- 修复帖子图片不加载问题：确认 `normalizePost`、`normalizeImage` 等函数已正确使用 `resolveImageUrl` 处理图片URL，数据规范化链路完整。
- 修复Web端举报管理页面按钮点击布局抖动问题：操作列「处理/收起」按钮统一固定宽度 `w-12` 并居中对齐，状态筛选按钮（全部/待处理/已处理）固定宽度 `w-16` 居中，切换状态时按钮宽度一致不再引起其他元素位移。

### 变更

- 删除小程序修改头像功能：个人中心头像仅作为展示，移除头像编辑按钮、`onAvatarTap` 上传函数、`avatarUploading` 状态及相关导入。
- 补充平台学校管理页面「新增学校」按钮和完整弹窗UI：表单包含学校代码、名称、省份、城市、详细地址、中心经纬度、地图缩放级别、Logo URL、品牌色、初始套餐、管理员邮箱、学校描述等字段，提交调用 `POST /platform/schools` 接口完成学校创建。

### 验证

- 前端 `npm run build` 构建成功，TypeScript 类型检查和 Vite 打包无错误。

## [2.2.30] - 2026-08-09

### 修复

- 修复小程序端AI搜索因全局15秒请求超时被前端断连，导致"没有找到相关内容"的问题；给request封装增加可选timeout参数，AI搜索单独设置60秒超时并显示loading遮罩，普通接口仍保持15秒超时不影响体验。后端实测AI搜索全链路（DeepSeek意图解析+Embedding生成+向量检索）耗时约4-5秒，性能正常。
- 修复阿里云短信核验接口对错误验证码抛出 `isv.ValidateFail` 时被错误返回为 500 的问题；错误验证码现在返回明确的"验证码错误"，其他短信服务异常返回可重试提示。
- 修复普通用户新增地点无需审核直接出现在公开列表的问题：公开地点列表接口仅返回 `is_verified=True` 的已核验地点；管理员创建地点直接通过核验，普通用户提交地点进入审核队列，返回明确提示，两端前端适配新响应结构显示对应成功提示。
- 修复帖子图片和头像预览不显示问题：后端 `UPLOAD_DIR` 配置从相对路径改为基于 backend 目录的绝对路径，确保无论启动目录如何，文件上传位置和静态文件挂载位置始终一致，图片/头像可正常访问。
- 修复切换学校后个人主页点击自己的帖子提示"资源不存在"问题：个人主页增加学校变化检测，切换学校后自动清空旧学校的帖子/浏览历史列表、重置分页、清零统计数字，并强制重新加载当前学校的数据，避免跨校帖子ID残留导致访问404。
- 修复帖子详情页举报弹层：点击选择举报类型时弹层自动关闭的问题。原因是`catchtap=""`空字符串无法正确阻止事件冒泡，点击类型选项时事件冒泡到遮罩层触发关闭。改为绑定空函数`stopPropagation`后事件正确拦截，可正常选择举报类型并填写理由提交。

### 变更

- 校园认证新增全局QQ邮箱白名单，所有学校（江南/复旦/浙大）均允许使用 `qq.com`、`vip.qq.com`、`foxmail.com` 完成身份认证，方便复赛评委使用个人邮箱演示。
- 取消演示验证码机制，仅 pytest 自动化测试环境在响应中返回验证码，opengauss 复赛环境改为通过 QQ 邮箱 SMTP 真实发送验证邮件。
- 前端移除固定显示的「演示环境验证码」区块，正常发送成功提示查收邮件，发送失败兜底弹窗显示验证码。
- Web端校园认证提示文案更新，地图页副标题简化。
- 后台管理菜单清理：移除「专题管理」和「反馈管理」两个已停用功能入口。
- 江南大学单独设置校管理员账号（手机号 `13900000000 / pass123`，角色 `admin`，昵称「江南大学运营组」），与平台超管（`13900000001`，角色 `super_admin`）明确区分；超管可见平台运营菜单组，校管理员仅可见本校运营功能。

### 验证

- 后端AI搜索实测：查询"有哪些二手物品在转让"耗时4.45秒，返回69条相关结果，意图解析正确，语义搜索正常。
- 手机号认证与短信 Provider 定向测试 15/15 通过。
- 后端SMTP配置验证通过，邮件服务可正常投递QQ邮箱验证邮件。
- 白名单逻辑验证：qq.com/foxmail.com 放行，gmail.com 等非白名单仍正确拦截。
- 后端服务正常启动监听 `0.0.0.0:8000`，局域网访问正常。
