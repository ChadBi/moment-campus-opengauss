...
- **校园热榜页视觉升级（HotRankingPage）**：Hero 头部橙渐变增层并加径向柔光光斑；"十大榜单"主标题字距正常化（去掉 tracking-[0.18em] 字帖感和硬 3px drop-shadow）；新增三列热度统计条（累计浏览/点赞/评论），万/k 自动格式化；
  Top1 冠军改为独立大卡（首图缩略或渐变封面 + 64×64 金渐变大奖章上移浮边 + 分类 #tag 从作者行下移单独一行）；Top2/3 双列次卡（银/铜渐变奖章 + 银蓝/铜色 halo）；4-10 改为紧凑横向列表，hover 时左 3px 橙条动画 + 徽章反色 + 标题变橙字；
  所有 Eye/Heart/Message 图标统一橙品牌色（不再只标浏览量）；整体包体积 16.76 kB / gzip 4.65 kB。

### 验证

- `frontend npm run build` TS 类型检查 + Vite 生产构建 0 错误 0 warning ✅
