...
- [x] **布局调整**：将发布页固定操作栏上移至自定义 TabBar 上方，并叠加底部安全区。
- [x] **内容留白**：增加页面底部预留空间，确保正文滚动到底时仍可看到并点击“存为草稿/提交审核”。
- [x] **安全区修正**：移除操作栏内部重复的底部安全区，避免按钮被额外向上顶出。
- [x] **相关验证**：小程序类型检查、格式测试通过；微信开发者工具刷新成功，发布页截图确认操作栏已位于自定义 TabBar 上方。
