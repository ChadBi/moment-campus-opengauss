...
- 待完成 8 项：R-01（口径对齐）、R-07（性能验证）、R-09（移动端/异常态）、R-10（产品说明书+截图）、R-11（演示视频）、R-12（Session ID）、R-13（社区作品帖）、R-14（飞书问卷提交）
- 关键路径：R-10 产品说明书 → R-11 演示视频 → R-12 Session ID 整理 → R-13 社区作品帖 → R-14 飞书问卷提交
...
- [x] **MP-PLAN-01** 完成微信小程序接入现状评估、官方规则调研、模块边界、准备清单、测试与发布方案（完成日期：2026-07-29）
  ...
  - 结论：新增独立 `miniprogram/` 原生 TypeScript 用户端，复用现有 FastAPI/openGauss；管理后台保留 Web；首期邮箱 JWT 登录且不依赖实时定位
- [ ] **MP-01** 完成小程序主体/类目/AppID/体验成员与服务器合法域名硬门槛验证（优先级 P0，截止日期：待确定）
- [ ] **MP-02** 创建 `miniprogram/` 工程骨架并实现请求层、JWT 刷新锁与学校上下文（优先级 P0，截止日期：待确定；依赖 MP-01）
- [ ] **MP-03** 实现首页、地图、搜索、详情、发布、上传、互动和协同验证关键闭环（优先级 P0，截止日期：待确定；依赖 MP-02）
- [ ] **MP-04** 完成开发者工具自动化、Android/iOS 真机、体验版、后端与 Web 回归测试（优先级 P0，截止日期：待确定；依赖 MP-03）
- [ ] **MP-05** 评审并按需实现微信一键登录、外部身份绑定和账号合并策略（优先级 P1，截止日期：待确定；不作为首期体验版前置）
