...
> 最后更新：2026-08-08（校园热榜页 UI 全面升级 v2.2.25）

## 2026-08-08 执行任务：校园热榜页 UI 全面升级
> 用户反馈：Web 端「校园热榜 / 十大榜单」页面整体视觉偏土（纯橙渐变 + 10 张全等卡片 + 重复文案），需要按 premium consumer 审美升级

- [x] **Design Read & 三 Dial 设定**：DESIGN_VARIANCE=7（Top1 大卡/Top2-3 次大卡/4-10 紧凑列表，打破 10 卡全等）；MOTION_INTENSITY=5（Tailwind transition +  hover 抬升/左亮条/橙条高亮）；VISUAL_DENSITY=4（Hero 多留白+列表紧凑形成对比）；统一软圆角系统 + 单橙品牌色 + 金银铜渐变辅助色
- [x] **Hero 头部重写**：橙渐变升级为三段层次感（`#FF8C5A→#FF7149→#EA5A2D`）+ radial 柔光光斑（替换两个空心圆圈剪贴画装饰）；删除重复三重"校园热榜"，改为导航栏小标签"校园热榜"+ 右侧 Sparkles 信息胶囊「近 7 天 · 浏览量 TOP 10」 + 主标题「十大榜单」（tracking-tight 大字距正常化，删除 3px 硬 drop-shadow 复古阴影）+ 副标题 + 三列热力度统计卡（累计浏览/点赞/评论，万/k 自动格式化）+ 右侧桌面端 TrendingUp 圆形浮标
- [x] **Top1 冠军卡**：单独独立形态，顶部带缩略图（有图用首图缩略图，无图用金渐变+大 Trophy 水印），左上角 **64×64 超大金奖章**（金色渐变+Trophy 图标+01+内阴影高光+4px 白环+负 mt-10 上移浮在缩略图边缘，视觉独占层级），作者+认证+时间与奖章并列，分类 #tag 移到作者下单独一行（不挤作者行），底部浏览/点赞/评论/Chevron 四个图标**全部统一橙品牌色**，大光晕暖阴影+hover 抬升
- [x] **Top2/3 并列卡**：`md:grid-cols-2` 双列，44×44 银/铜渐变奖章（`#E8EEF5→#BCC9D7` 银色、`#FFC6A8→#E07A49` 玫瑰铜色），单独 halo 光晕，时间贴右，标题/分类/内容/数据保留但字号缩小一档
- [x] **Top4-10 紧凑列表**：单卡片容器包裹，header「第 4-N 名 / 紧凑视图」，每行横向三列布局：**排名徽章**（04-10 橙底白字/hover 时反色成深橙底白字）+ **标题 & 分类小标签+作者·时间·地点** + **浏览量+Chevron 箭头**；hover 时背景渐变 from `#FFF1E6` + **左边 3px 垂直高亮橙条 scale-y-0→100**（不是 border-left 静态，动画显现）+ 徽章反色 + 标题变橙字 + Chevron 右滑
- [x] **Pre-flight 规范检查 & 构建验证**：0 个 em-dash / 1 个 eyebrow 符合上限 / 无 zigzag 布局重复 / 无 section 编号 / 无装饰性文字条 / 字号层次 6 档 / 圆角全部 soft 系；`frontend npm run build` TS 类型+Vite 构建 0 错误 0 warning ✅
