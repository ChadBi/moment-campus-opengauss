...
- [x] **开通向导页面新增「新增学校」按钮**：在「选择或创建学校」步骤卡片头部添加主按钮，点击打开快速创建弹窗
- [x] **快速创建学校弹窗**：包含学校代码、名称、省份、城市、地址、中心经纬度、地图缩放等核心字段；Logo/品牌色/套餐/管理员邮箱等非必要字段提示可在「学校管理」后续完善
- [x] **创建后自动选中**：学校创建成功后自动刷新学校列表并选中新创建的学校，无需手动选择即可继续开通流程
- [x] **空状态引导**：当平台暂无学校时显示友好空状态，引导用户点击按钮创建第一所学校
- [x] **步骤描述更新**：页面流程描述更新为「新增学校 → 下载模板 → 上传 CSV → 预览校验 → 批量导入」
- [x] **修复无关TS错误**：移除 AdminLocationsPage.tsx 中未使用的 Info 导入
- [x] **端到端验证**：超管账号登录后，开通向导页面可正常打开新增学校弹窗，学校管理页面原有新增功能正常
- [x] **前端构建通过**：`npm run build` 成功

## 2026-08-09 整理：仓库整理与版本升级 2.3.0

> 里程碑版本：统一长期滞后的版本号、补齐工程规约、清理磁盘垃圾、纠错过时文档。详见 [AIwork/仓库整理与版本升级2.3_任务报告.md](AIwork/仓库整理与版本升级2.3_任务报告.md)。

- [x] **版本号统一**：README.md（2.0.0）、frontend/package.json（2.1.0）同步至 2.3.0；CHANGELOG.md 顶部新增 2.3.0 版本块
- [x] **工程规约补漏**：.gitignore 补加 `delete/` 规则（AGENTS.md 规约要求但此前缺失）
- [x] **文档纠错**：修正 AGENTS.md「根目录两个 HTML 文件」过时描述（已于 2026-07-26 归档至 `rubbish/`）；为 `docs/18_项目现状说明.md` 添加废弃声明
- [x] **磁盘清理**：清理全仓 `__pycache__`（408 个目录，含孤儿 .pyc）及 `rubbish/` 内日志/截图/临时文件
...
> 问题原因：登录成功后直接navigate跳转页面，但`currentSchoolCode`仍然是之前登录过的旧学校（如先登录江南大学，再登录复旦大学时，currentSchoolCode还是jiangnan），导致跳转后首次API请求用错误的学校头发送失败，随后useSchoolSync检测到无权限才自动切换学校并弹出"您无当前学校的权限"提示，需要再点一次登录才能正常进入。

- [x] **修复登录页学校上下文设置**：LoginPage登录成功后，在跳转前先根据返回的`user.school_id`从公开学校列表找到对应学校，调用`setCurrentSchool()`设置正确的当前学校
- [x] **超管跳过设置**：超管账号不绑定特定学校，保持原有跳转逻辑不变
- [x] **前端构建验证**：`npm run build`成功通过

## 2026-08-09 修复：任务记录页面403权限错误

> 问题原因：「任务记录」是跨校平台级功能（后端使用`SuperAdminDep`仅超管可访问），但前端菜单未标记为`superAdminOnly`，校管理员菜单中能看到该页面，访问时返回403"没有权限执行此操作"。

- [x] **修复菜单权限标记**：将「任务记录」菜单项移动到平台运营组，标记`superAdminOnly: true`，校管理员菜单不再显示该选项
- [x] **完善路由守卫**：将`/admin/jobs`加入平台专属路由判断；补充校管理员访问平台路由时自动重定向回`/admin`管理首页（之前只有超管访问校级页面的重定向，缺少反向重定向）
- [x] **前端构建验证**：`npm run build`成功通过
...
- [x] **江南大学独立校管理员**：新增江南大学校管理员账号 `13900000000 / pass123`，角色 `admin`，昵称「江南大学运营组」，与平台超管（`13900000001`，角色 `super_admin`）区分开
- [x] **验证功能正常**：
  - 举报管理页面正常打开（菜单存在，当前0条举报记录）
  - 任务记录页面正常（定时任务运行记录，当前0条）
  - 开通向导页面正常（可选择学校、下载模板、上传CSV、预览、导入）
...
- [x] **TS添加空函数**：`miniprogram/pages/post-detail/post-detail.ts` 中新增 `stopPropagation() {}` 空方法用于阻止事件冒泡
- [x] **WXML修正事件绑定**：`miniprogram/pages/post-detail/post-detail.wxml` 中将 `catchtap=""` 改为 `catchtap="stopPropagation"`，正确阻止弹层内容区域点击事件冒泡到遮罩层
- [x] **全局排查**：grep确认全项目仅有这一处使用了`catchtap=""`空绑定，无其他同类问题
...
- [x] **request封装支持自定义超时**：`miniprogram/services/request.ts` 的RequestOptions新增`timeout`可选参数，主请求和401重试请求都使用传入的timeout，默认仍保持15秒
- [x] **AI搜索单独设置长超时**：`miniprogram/services/search.ts` 中 `aiSearch()` 调用改为显式传入 `{ timeout: 60000, loading: true }`，AI搜索给60秒足够时间，同时自动显示loading遮罩；普通接口仍保持15秒超时，不影响其他功能体验
- [x] **后端耗时验证**：实测HTTP接口AI搜索总耗时4.45秒，返回69条结果，意图解析、Embedding生成都正常，无后端性能问题；Web端axios默认无超时，不受影响
- [x] **问题已定位修复**：现在在微信开发者工具重新编译小程序即可正常使用AI搜索

## 2026-08-09 执行任务：阿里云错误验证码处理修复
> 修复真实阿里云短信核验接口在验证码填写错误时抛出异常，导致小程序显示服务器内部错误的问题。

- [x] **错误码转换**：识别阿里云 `isv.ValidateFail` 和“验证失败”响应，统一返回 400“验证码错误”。
- [x] **服务异常隔离**：阿里云网络或其他核验异常返回短信服务暂不可用，不再穿透为 500。
- [x] **真实场景回归**：新增阿里云 Provider 异常分支测试，手机号认证与短信 Provider 定向测试 15/15 通过。
...
- [x] **后端快速登录接口**：新增 `POST /auth/wechat/login`，按微信 OpenID 查找已绑定身份，直接签发小程序登录会话。
- [x] **首次登录分流**：未找到微信身份时继续进入手机号短信绑定流程；已绑定账号不再要求输入手机号或重新选择学校。
- [x] **一次性 code 修复**：小程序每次提交短信绑定（包括失败重试）都重新调用 `wx.login()`，避免复用已消费 code。
- [x] **登录状态同步**：快速登录和短信绑定共用学校绑定、缓存切换与个人中心跳转逻辑。
- [x] **定向验证**：手机号认证测试 8/8 通过，小程序类型检查、格式检查和登录页 WXML 编译通过。

## 2026-08-09 执行任务：大规模测试数据重新生成
> 原seed数据量不足，重新生成：3所学校×50用户=150人，每校500帖共1500帖，自动生成Embedding向量。

- [x] **数据量扩充**：每校50用户（共150人，约61%已校园认证），每校500帖子（共1500帖，覆盖全部6种状态：published/draft/pending/expired/conflict/archived）
- [x] **互动数据生成**：22369条点赞，4231条评论（幂律分布，头部帖子十几条，尾部0-2条），1105条协同验证（confirmation/refutation两类互斥）
- [x] **地点评价**：39个地点共582条评价，每个地点至少10条，评分与文本自然分布
- [x] **内容真实感优化**：构建口语化素材库，标题/正文/评论/评价全部使用自然口语，无Markdown格式，加入语气词扰动
- [x] **手机号动态生成**：改用`seq + phone_prefix`动态生成三校预置用户手机号，避免硬编码位数错误
- [x] **Embedding自动生成**：数据提交后自动调用embedding服务，为1410条已发布帖子生成512维向量，AI语义搜索立即可用
- [x] **边界问题修复**：修复互动数据时间计算边界（帖子创建时间过近时randint空范围）
- [x] **数据验证**：脚本运行成功，统计信息符合预期：用户150、帖子1500、点赞22369、评论4231、验证1105、地点评价582、Embedding 1410


## 2026-08-09 执行任务：发帖 AI 真实链路修复
> 修复真实推理模型因输出 token 上限不足导致的空响应降级，并同步小程序发帖上下文与敏感信息检测。

- [x] **真实模型输出**：发帖建议专用 `max_tokens` 从 1200 提升至 2400，避免推理 token 耗尽后 JSON 内容为空。
- [x] **降级文案**：发布建议按场景显示“仅返回敏感信息检测结果”，不再误显示“已降级普通搜索”。
- [x] **小程序请求上下文**：AI 辅助请求补充分类、地点、联系方式、失物类型和截止时间。
- [x] **表单状态保护**：模型未返回分类时保留用户已选分类；模型明确返回非法分类时仍按白名单丢弃。
- [x] **敏感信息检测**：手机号不再被 QQ 数字规则重复命中。
- [x] **真实链路验证**：真实 8000 接口返回结构化建议，手机号仅命中 phone；定向后端测试 52/52、小程序类型/格式检查和发帖页 WXML 编译通过。

## 2026-08-09 执行任务：游客学校切换与发帖入口
> 游客可以自由选择本地浏览学校；点击底部发布按钮时只弹窗提醒登录，不切换页面。

- [x] **游客切换学校**：未登录确认学校时只更新 `campusStore` 和本地 `school_code`，不调用需要登录的学校绑定接口；登录用户继续保留账号学校绑定逻辑。
- [x] **游客发布入口**：游客点击底部发布按钮时立即弹窗提示登录，不执行页面跳转；登录用户正常进入发布页。
- [x] **写操作登录兜底**：保存草稿、提交审核继续使用 `requireLogin`，未登录时不发起创建帖子请求。
- [x] **小程序静态验证**：`npm run typecheck`、`npm run test:format` 通过；局部 WXML/WXSS 编译请求受微信侧 `-80408` 频率限制，未将其记为通过。

## 2026-08-09 执行任务：学校选择页界面优化与搜索
> 在现有学校绑定逻辑上优化小程序学校选择页的视觉层级，并增加学校名称、城市、地区和 code 搜索。

- [x] **视觉层级优化**：重做页面头部、搜索区域、学校卡片、选中态和底部确认栏，保持水墨校园风格并强化选中反馈。
- [x] **学校搜索**：增加实时搜索栏，支持学校名称、城市、地区、描述和学校 code 筛选，支持清空关键词和无结果状态。
- [x] **交互状态同步**：筛选结果中重新选择学校时同步完整列表和底部确认栏，避免搜索后选中态丢失。
- [x] **小程序验证**：类型检查、格式检查、学校选择页 WXML/WXSS 编译、模拟器页面打开和截图视觉复核通过。
- [ ] **自动输入验证**：微信开发者工具 automator 输入接口被当前调试会话占用而超时，需用户在工具或真机手动输入关键词完成最终交互验收。

## 2026-08-09 执行任务：小程序微信短信绑定登录
> 微信手机号授权组件受小程序主体认证限制，改为微信会话初始化后使用手机号和真实短信验证码绑定 OpenID 并登录；保留原授权手机号接口供后续恢复。

- [x] **后端短信绑定接口**：新增 `POST /auth/wechat/sms-login`，依次完成微信 `code2Session`、`login` 用途短信核验、手机号账号复用/创建和 `wechat_miniprogram` 身份绑定。
- [x] **账号安全规则**：只允许无手机号的历史微信壳账号合并到手机号账号；微信已绑定其他真实手机号时拒绝静默换绑。
- [x] **后端定向测试**：`tests/test_phone_auth.py` 7/7 通过，覆盖新账号、已有手机号复用、无密码状态、微信身份记录和错误验证码拒绝；未执行后端全量测试。
- [x] **小程序登录界面**：改为微信初始化、手机号输入、短信验证码和绑定登录两阶段流程；不再依赖 `getPhoneNumber` 授权弹窗。
- [x] **小程序验证与交付**：类型检查、格式检查、微信开发者工具 WXML/WXSS 编译、控制台检查及任务文档已完成；自动点击因当前真机调试会话占用 automator 而超时，已在报告中如实记录。
- [x] **真实短信发送验收**：补齐 `backend/.venv` 已声明但未安装的阿里云号码认证 SDK，向 `153****0630` 发送 `login` 用途真实短信成功；正式库生成 `provider=aliyun` 的未使用核验流水，响应未暴露 Mock code。
- [x] **登录学校选择**：删除“微信身份已确认”提示块，替换为公开学校目录选择器；新账号使用所选学校创建唯一 active membership、设置 `user.school_id` 与 `registration_school_id`。
- [x] **绑定学校状态同步**：已有账号不会因登录页选校而切校；登录后按响应 `user.school_id` 同步 `campusStore`，个人主页切校后同步 `authStore.user.school_id`；首页监听学校变化并绕过 30 秒节流重载分类与内容。
- [x] **定向验证**：后端手机号认证测试 7/7、小程序类型/格式检查、依赖完整性检查、登录页 WXML/WXSS 编译和控制台检查通过；未执行后端全量测试。
- [ ] **真机完整登录验收**：真实短信已发送，但未取得用户收到的 6 位验证码，尚未代替用户完成验证码核验、OpenID 绑定和进入个人中心。
...
- [x] **用户配置**：超管使用 `13900000001`；三校管理员/普通用户使用固定手机号段；新增江南大学微信演示账号 `13800138000`。
- [x] **认证状态**：seed 新用户的历史 `users.email` 全部置空；已认证用户拥有唯一 `education_email`，未认证用户不写教育邮箱；微信演示账号无密码。
...
- [x] **真实服务配置入口**：在本地 `backend/.env.opengauss` 增加阿里云短信和微信真实登录变量，并在 `.env.opengauss.example` 补充微信变量；真实凭据待本地填写后才能启动 production 链路。
- [x] **短信模板按用途拆分**：新增注册、登录、设置密码和解除教育邮箱绑定四个阿里云模板配置，旧通用模板字段保留为兼容兜底。
...
## 2026-08-08 执行任务：手机号主账号、微信手机号登录与教育邮箱认证改造
> 认证体系从邮箱/密码登录切换为手机号业务唯一身份；教育邮箱仅用于校园认证，微信小程序通过授权手机号直接登录。

- [x] **后端数据与迁移**：`users.phone` 唯一业务键、`education_email` 可空唯一、`password_hash` 可空；历史邮箱回填教育邮箱并清理 `email_password` 身份，保留 `users.id` 物理主键和微信身份记录。
- [x] **短信服务**：新增 `sms_verifications`，验证码只保存哈希；6 位数字、5 分钟有效、60 秒发送间隔；本地 Mock Provider 与生产阿里云 Provider 均按环境变量配置。
- [x] **Web 认证**：手机号短信/密码登录、手机号短信注册、删除邮箱找回密码入口；小程序无密码账号可在个人中心一次性设置密码。
- [x] **微信小程序认证**：`/auth/wechat/phone-login` 使用微信手机号授权自动创建/登录账号，保留 `wechat_miniprogram` 身份并处理手机号账号合并；移除传统邮箱绑定注册入口。
- [x] **教育邮箱认证**：提交当前注册学校教育邮箱、邮件验证码确认、邮箱唯一约束；解绑前必须通过当前手机号短信验证码。
- [x] **演示数据**：清空并重建 openGauss 三校数据，演示账号统一使用生成手机号和 `pass123`；新注册账号默认未认证。
- [x] **定向验证**：`tests/test_phone_auth.py` 4/4 通过；`frontend npm run build`、小程序 `typecheck`/`test:format`、微信开发者工具刷新/页面打开/WXML/WXSS 编译通过；未执行后端全量测试。
- [x] **文档与交付**：同步更新认证流程、数据库、API、安全与路线文档，新增中文任务报告和 CHANGELOG v2.2.22。
...
- [x] **注册学校固化**：用户新增 `registration_school_id`，注册时记录；历史用户通过迁移回填，`school_id` 继续表示当前学校。
- [x] **认证状态保留**：切换学校不再清空 `campus_verified`，原注册学校认证状态切回后仍然有效；认证验证码始终绑定注册学校。
- [x] **后端只读门禁**：普通用户在非注册学校不能认证、发帖、评论、点赞、协同验证、地点评价、资料提议或新增地点；管理员能力保持现有角色体系。
- [x] **Web/小程序联动**：非注册学校隐藏校园认证卡片和个人资料“已认证”徽标，写操作入口显示“当前学校仅支持浏览”，切回注册学校恢复。
- [x] **定向验证**：后端校园认证/只读权限 17 passed、1 skipped；Web 构建、小程序类型检查和格式检查通过；微信工具地点页 WXML 编译通过。

## 2026-08-08 执行任务：修复新增地点按钮文字对齐

- [x] **定位对齐原因**：确认 `plus` 图标未在小程序 icon 组件中注册，空白图标仍占用布局宽度，导致按钮文字视觉偏移。
- [x] **移除无效占位**：地图页和全部地点页移除无效的 plus 图标节点，让“新增地点”文字单独居中。
- [x] **定向验证**：地点页、地图页 WXML/WXSS 编译通过，开发者工具截图确认按钮文字居中，console 错误检索为空。

## 2026-08-08 执行任务：新增地点表单字段优化

- [x] **移除楼层字段**：新增地点表单不再要求填写楼栋、区域和楼层，避免把校园地点限制在不稳定的室内层级信息上。
- [x] **增加场所类型**：使用教学楼、食堂、宿舍、运动场、服务点、公共空间、其他等通用类型，辅助用户快速理解地点用途。
- [x] **保留地点描述**：描述字段继续保留，并提示填写开放时间、使用规则和联系方式；场所类型以兼容格式写入现有描述接口。
- [x] **定向验证**：完成小程序类型检查、格式检查和地点页 WXML/WXSS 编译检查。

## 2026-08-08 执行任务：新增地点地图选点与入口布局优化

- [x] **地图入口调整**：删除地图顶部“已更新”状态，将“新增地点”移动到原状态位置；保留“全部地点”和缩放控件在地图工具栏。
- [x] **地点页按钮居中**：新增地点按钮固定高度并统一水平、垂直居中，标题区整体垂直对齐。
- [x] **地图选点表单**：移除用户手填经纬度输入，新增校园地图选点区域；点击地图后自动更新 marker、纬度、经度和已选状态。
- [x] **提交校验**：未在地图上选点时禁止提交并给出提示，避免新地点被误提交到学校中心点。
- [x] **定向验证**：小程序类型检查、格式测试、地图页/地点页 WXML 与 WXSS 编译通过；开发者工具截图确认入口布局和地图弹窗；自动化注入地图点击事件后状态、坐标和 marker 均正确更新。

## 2026-08-08 执行任务：彻底修复小程序 TabBar 跨页闪烁与重复加载

- [x] **切换时序修复**：识别每个 Tab 页各自持有并缓存独立自定义 TabBar 实例；切换时不再提前修改源页面高亮，目标页面显示后只由自己的 `onShow` 一次性同步，消除“目标项 → 上一项 → 目标项”的实例竞争。
- [x] **真机合成优化**：移除 TabBar 高亮背景过渡、点击态滞留和 `backdrop-filter` 模糊层，改用稳定纯色背景，避免固定导航覆盖原生地图组件时触发额外重合成。
- [x] **地图重复加载**：按学校代码区分真正的学校切换和同校详情补全，同校状态更新不再清空 marker；增加同版本请求锁，首次进入地点接口由连续 2 次降为 1 次。
- [x] **个人页返回体验**：个人页增加 60 秒数据复用窗口，后台刷新时保留已有帖子/历史列表，返回页面不再先清空再整组请求。
- [x] **定向验证**：小程序类型检查、格式测试、TabBar WXML/WXSS 编译均通过；开发者工具连续验证 6 次跨 Tab 切换，源页面高亮全程不变、目标页首次读取即为正确索引；地图 15 个 marker 正常且仅 1 次地点请求；个人页短时间返回新增请求数为 0；console 错误检索为空。

## 2026-08-08 执行任务：收敛小程序 TabBar 导航状态

- [x] **唯一状态源**：新增 TabBar 导航状态工具和全局目标索引，切换前同步目标高亮，失败时恢复原索引。
- [x] **统一页面实例**：首页、地图、搜索、发布、我的只通过统一同步函数更新各自 TabBar 实例，不再各自维护硬编码状态；页面内和登录/注册/地点操作的 Tab 跳转统一走导航工具。
- [x] **相关验证**：小程序类型检查、格式测试、原始 `wx.switchTab`/高亮写入审计和 Git 差异检查通过；真实设备验证仍需开发者工具授权后进行。

## 2026-08-08 执行任务：修复小程序首次进入页面白屏与切换闪烁

- [x] **首次加载**：移除 `requiredComponents` 懒加载，避免自定义组件和 TabBar 在真机首次进入时才加载导致白屏。
- [x] **首帧显示**：页面切换动画取消透明度淡入，保留轻微位移动画，避免页面首帧不可见造成闪烁。
- [x] **相关验证**：小程序类型检查、格式测试、`app.json` 解析和 Git 差异检查通过；开发者工具逐页验证因本机 CLI 等待授权暂未完成。

## 2026-08-08 执行任务：修复小程序导航切换闪动与返回重载

- [x] **TabBar 跳转**：增加重复点击锁，等待 `wx.switchTab` 成功后再更新高亮；移除失败后强制回首页的兜底跳转。
- [x] **首页返回**：保留已有信息流，避免每次 `onShow` 清空页面；增加 60 秒刷新窗口，下拉刷新仍然强制更新。
- [x] **相关验证**：小程序类型检查、格式测试、自定义 TabBar WXML 编译、开发者工具刷新/打开首页通过，模拟器错误日志为空。

## 2026-08-08 执行任务：微调发布页 AI 助手卡片样式

- [x] **信息层级**：收敛卡片边框和阴影，强化标题与副标题的阅读层级。
- [x] **操作按钮**：调整 AI 建议按钮尺寸、间距和字距，改善窄屏下的视觉拥挤。
- [x] **相关验证**：小程序类型检查、格式测试、WXML/WXSS 局部编译通过；模拟器刷新后自动化等待仍受工具超时影响。

## 2026-08-08 执行任务：升级 AI 助手标题与正文优化能力

- [x] **后端输出**：AI 发布建议新增可选 `optimized_title` 和 `optimized_content`，保留原有建议字段与旧模型输出兼容性。
- [x] **安全约束**：提示词明确只润色表达、不新增事实；后端对标题和正文长度进行截断校验，并扩大生成预算。
- [x] **小程序/Web**：发布页展示优化标题和正文，支持分别采纳；小程序一键应用会同时应用标题、正文和原有分类建议。
- [x] **定向验证**：后端 AI 发布建议 23/23 通过；Web `npm run build`、小程序类型检查和格式测试通过。

## 2026-08-08 执行任务：移除用户内容订阅功能

- [x] **小程序**：移除地点/专题订阅按钮、订阅服务、订阅通知偏好开关及相关类型归一化代码。
- [x] **Web**：移除专题订阅按钮、我的订阅卡片、订阅服务和用户订阅类型；通知偏好保留互动/审核/治理/系统等开关。
- [x] **兼容边界**：保留后端用户订阅接口、历史通知数据和平台商业订阅管理，不再从用户端发起新的内容订阅。
- [x] **相关验证**：小程序类型检查、格式测试和 Web `npm run build` 均通过。

## 2026-08-08 执行任务：放开 qq.com 注册与校园认证

- [x] **小程序/Web 注册**：前端域名校验允许 `qq.com`，并更新输入提示与错误提示。
- [x] **后端注册/认证**：沿用统一邮箱域名校验白名单，注册和校园验证均允许 `qq.com`。
- [x] **定向验证**：邮箱注册、微信注册、发送校园验证码、完成校园认证 4 条后端用例通过；小程序类型/格式检查和 Web 构建通过。

## 2026-08-08 执行任务：优化地点资料编辑入口

- [x] **详情页**：移除底部常驻“补充地点资料”表单，在“已审核资料”标题旁增加“补充/修改”小按钮。
- [x] **编辑弹窗**：点击后通过底部弹窗展示登录/认证状态和资料提议表单，提交成功自动关闭并提示审核状态。
- [x] **地图快捷入口**：地图地点信息卡的已审核资料区域增加同一“补充/修改”入口，跳转到对应地点详情。
- [x] **相关验证**：地点资料提议与管理员审核 2 个后端用例通过；小程序类型检查、格式测试通过。

## 2026-08-08 执行任务：修复发布页底部操作栏遮挡

- [x] **布局调整**：将发布页操作栏改为页面内容末端的普通文档流，不再固定在视口底部。
- [x] **内容留白**：仅在页面末尾为固定 TabBar 预留滚动空间，滚到底部时操作按钮仍可见、可点击。
- [x] **底部定位复核**：开发者工具截图确认操作栏位于联系方式等内容之后，并随页面一起滚动。
- [x] **AI 入口优化**：参考 Web 端 AI 辅助发布建议卡片，将 AI 入口移到顶部并补充标题、用途说明和“AI 建议/重新生成”操作。
- [x] **操作栏视觉**：移除操作栏外层白色填充，统一按钮色值并强制“存为草稿”单行显示。
- [x] **相关验证**：小程序类型检查、格式测试通过；微信开发者工具发布页打开成功，console 无错误匹配。

## 2026-08-08 执行任务：新增校园地点入口

- [x] **地图与地点页入口**：地图工具栏和全部地点页均新增“新增地点”按钮，统一跳转/打开同一地点表单。
- [x] **提交表单**：支持名称、描述、楼栋、楼层和经纬度，默认填当前学校中心点；提交后刷新列表并进入地点详情。
- [x] **权限与核验**：普通用户需完成校园认证，管理员可直接提交；新地点保持 `is_verified=false` 进入管理员核验队列。
- [x] **相关验证**：地点权限和地点创建定向后端测试 8/8 通过；小程序类型检查、格式测试通过。

## 2026-08-08 执行任务：地图地点标签优化

- [x] **常驻地点标签**：地图所有 marker 的 callout 改为 `ALWAYS` 展示，不再需要点击后才显示。
- [x] **标签信息**：callout 统一展示地点名称和评分，暂无评分时显示“暂无评分”；小程序类型检查通过。

## 2026-08-08 执行任务：修复校园动态帖子点击参数错误

- [x] **事件冲突修复**：`post-card` 不再使用原生 `tap` 作为自定义事件，改为 `posttap` 并阻止根节点事件冒泡，避免首页先跳转到 `id=undefined` 的详情页。
- [x] **跳转参数防护**：首页、专题详情和地图相关帖子统一使用独立事件并校验数值 ID；小程序 `typecheck` 与格式测试通过。
- [ ] **微信模板 CLI 编译**：开发者工具 CLI 并发及单次 `compile_wxml` 调用均超时，后续通过开发者工具刷新/编译继续验证。
...
- [x] 地图抽屉固定在底部导航栏上方，修复内容被导航遮挡和地点帖子无法滚动的问题
- [x] 地图地点详情改为单一连续滚动内容流，标题、评分、摘要、帖子和详情按钮不再分成固定区与帖子内滚动区
- [x] 完成地图地点链路复核：学校中心坐标、15 个静态地点标记、地点详情/评价/相关帖子并行请求、半屏抽屉滚动/展开/关闭和快速切换请求隔离均通过
- [x] 修正地图面板相关帖子数量读取：改用 `GET /posts?location_id=...` 的 `total`，避免后端地点缓存 `post_count=0` 导致“有帖子却显示 0 条”
- [x] 修复微信开发者工具子包页面 WXSS 编译阻断：本地 `project.private.config.json` 关闭 Skyline 后，地图可正常跳转地点详情，学校选择/地点详情子包不再显示 WXSS 错误

- [x] 修复模式切换链接 WXML 换行造成的文本基线偏移，改为单行文本节点并重新截图确认两段文案对齐
- [x] 统一注册页底部“还没有账号？/注册新账号”模式切换的行高、基线和居中对齐，绑定模式截图验证通过
- [x] 修复登录页“以访客身份继续浏览”点击后无法切换首页的问题：`switchTab` 失败时回退 `reLaunch`，微信开发者工具自动化已验证进入首页
- [x] 微信未绑定用户改为直接进入主包邮箱注册页；注册表单补齐教育邮箱、昵称、学校、密码和确认密码，执行学校官方邮箱域名校验并移除动态导入提交失败
- [x] 小程序品牌区统一复用根目录 `logo.png`（同步放入 `miniprogram/assets/logo.png`），关于页补齐 `icon` 组件注册并修复图标占位；关于页与登录页已通过编译和模拟器截图验证
- [x] 小程序个人中心移除身份管理、设备管理和意见反馈入口及页面，清理对应弹窗、服务与过时协议文案
- [x] 小程序个人中心 WXML/WXSS、关于页和用户协议编译通过，模拟器截图确认入口已消失；四个设置页面已移至主包并通过自动化导航

- [x] 修复小程序身份管理/设备管理未携带登录凭据导致 401 清理登录态的问题；移除已废弃的个人中心“订阅管理”入口与页面
- [x] 补录校园认证真实登录态 E2E：教育邮箱登录、发送 6 位验证码、输入确认成功；普通邮箱被域名校验拦截

## 2026-08-07 执行任务：首页推荐收敛与校园热榜

- [x] 首页推荐区改为只展示一条精选卡片，推荐接口仍独立加载，不再把 5 条推荐拉成长列表
- [x] 首页区块顺序调整为“校园热榜 → 为你推荐 → 校园动态”，Web 与小程序保持一致
- [x] 热榜入口增加子按钮点击与导航失败反馈；首页预览卡片、Web 预览和完整榜单统一两行标题高度并对超长标题做省略
- [x] 热榜详情页由 subpackage 调整为主包页面，避免开发者工具子包导航超时影响“查看榜单”
- [x] 热榜页移除“+ 榜”装饰，保留 Top10 语义并简化榜单头图
- [x] 热榜主标题统一改为“十大榜单”，同步小程序与 Web 文案
- [x] 个人中心设置项图标统一固定槽位；删除重复退出按钮，将“退出全部设备”改为普通退出登录并移除小程序对应接口调用
- [x] 校园身份认证统一改为邮箱发送 6 位数字验证码，移除验证链接流程；当前学校卡片改为学校名与说明分行显示
- [x] 移除首页快捷入口区域，保留校园地点入口与普通校园动态信息流
- [x] 后端 `/posts` 新增 `sort=views`，支持已发布帖子按浏览量降序并保留当前学校隔离
- [x] 热榜统一读取最近 7 天时间窗口内的 Top10，首页使用横向滚动卡片，单条卡片可进入帖子详情
- [x] 小程序新增“校园热榜”独立页面，展示排名、作者、分类、摘要、地点和浏览/点赞/评论数据
- [x] Web 首页同步热榜横向预览并新增 `/hot-ranking` 独立榜单页，保持两端交互和口径一致
- [x] 兼容带时区 ISO 时间筛选，避免 openGauss `timestamp without time zone` 比较返回 500；新增后端排序与时区回归测试
- [x] 小程序 typecheck、format；Web build；后端 `test_search.py` 25 项通过；首页 WXML/WXSS 与热榜页面编译通过
- [x] 微信开发者工具中“查看榜单”已通过主包页面自动化导航复核，榜单接口返回 200，点击热榜条目可进入帖子详情；真机验证仍待补充
...
- [x] 发帖/编辑流程与地图地点面板已在 P2/P3 完成，待按阶段补充运行态验收记录
...
- [x] 小程序 `npm run typecheck`、`npm run test:format` 通过
- [x] 地图标记视觉与地点相关帖子面板已在 P3 完成，待按阶段补充运行态验收记录

## 2026-08-07 执行任务：Web/小程序一致性重构 P3-P4

- [x] 地图改用学校静态中心坐标与 `/locations`，移除旧 `/map/markers` 数据依赖
- [x] 已核验地点使用湖蓝水滴标记，待核验地点使用琥珀标记，选中地点高亮并显示圆角 callout
- [x] 点击地点并行读取详情、评价和 `GET /posts?location_id=...`，请求带学校与版本校验，旧响应不会覆盖新面板
- [x] 地图底部面板固定在自定义 TabBar 上方，展示稳定资料、AI 摘要、评分、来源数量、相关帖子和真实空状态
- [x] 首页已验证同时请求推荐（5 条）与普通信息流，地图已验证相关帖子请求和真实帖子展示
- [x] 统一帖子分类归一化（含 `category.code`）与分类列表归一化，失物类型识别不再依赖旧字段
- [x] 移除小程序专题详情的订阅入口，订阅 service 与订阅管理过滤 `target_type=topic`；保留分类/地点订阅
- [x] 地图、首页、发布、编辑相关 WXML/WXSS 编译通过；小程序 typecheck/format 通过
- [x] 视觉验收：地图浮层/水滴标记与学校选择页卡片通过模拟器截图复核，修复默认校徽文字换行
- [x] 微信开发者工具登录态已验证：认证用户创建草稿 → 冷启动恢复会话 → 编辑草稿 → 提交审核，接口链路通过（草稿 86）
- [x] 本地后端重启后 `/api/v1/schools/current/settings` 已出现在 OpenAPI，返回学校匿名/评论/图片上限等设置；此前 404 判定为旧进程未加载最新路由
- [x] 登录态 E2E：点赞、协同验证、评论、举报均返回 2xx；未调用 `/posts/{id}/interactions` 旧路径
- [x] 登录态 E2E：通知列表与 `PUT /notifications/read-all` 返回 200，未读数按 `unread_count` 读取
- [x] 登录态 E2E：分类订阅创建 201、取消 200，数据使用 `target_type=category`；专题详情无订阅入口
- [x] 登录态 E2E：江南大学 → 复旦大学 → 江南大学切换成功，`/schools/current` 中心坐标与地图刷新均按学校代码更新

## 2026-08-07 执行任务：小程序冷启动会话与本地路由复核

- [x] 非认证请求在 access token 仅存内存且页面重开时，先使用持久化 refresh token 恢复会话，避免私有草稿详情被无认证 404 误判为不存在
...
- [x] **MP-CONTRACT-P5-D** 为地图与地点列表补充显式学校租户和请求版本校验，防止切校后旧地点响应覆盖新学校。
- [x] **MP-CONTRACT-P5-E** 为仍含早期定位/附近口径的基线文档增加历史/已废弃声明，现行文档统一指向 AI 地点摘要方案与小程序契约矩阵。
- [x] **MP-CONTRACT-P5-F** 修复登录切换账号的请求层 token 串用；移除运行时不可用的 `URLSearchParams`，统一查询参数编码；通知/订阅页补齐登录生命周期竞态加载；专题订阅入口保持已删除。
- [x] **MP-CONTRACT-P5-G** 微信小程序真实 E2E 补跑：发布帖子 1517、双账号互动、通知列表与全部已读、分类订阅和地点订阅请求通过；远端地点详情路由 404 已如实记录，未将地点详情展示和专题订阅伪报为完成。
- [x] **MP-LOC-DETAIL-FIX** 修复地点详情弹层：远端详情/评价接口 404 时回退展示当前学校地点基础资料；移开页面入场动画对 fixed 弹层的影响，使弹层固定在当前可见视口底部，并使用面向用户的降级提示记录后端部署缺口。
- [x] **MP-LOC-ENV-LOCAL** 小程序环境地址按微信运行版本自动切换：开发者工具先走本地后端，体验版/正式版再走线上地址，保留 `__ENV__` 手动覆盖。

- [x] **MP-UI-REGISTER-MAP-SHEET** 修复注册页表单与地图地点抽屉交互（2026-08-07）：
  - 注册/绑定表单合并为一个连续卡片，统一输入框盒模型、宽度和溢出边界，修复中间输入框超出父容器的问题。
  - 地图地点抽屉半屏状态下整张窗口响应拖拽并实时跟随手指位移，上滑后切换全屏；全屏状态才启用单一内容 `scroll-view`，避免帖子区域和外层窗口分裂滚动。
  - 切换地点、切换学校和关闭抽屉时重置拖拽偏移与状态。
...
- [x] **MP-UI-PROFILE-POLISH** 优化小程序个人中心视觉与头像入口（2026-08-07）：
  - 个人资料头部改为稳定墨青纯色，移除刺眼渐变；头像相机按钮固定在头像右下角并接入相册/相机选择与头像上传接口。
  - 加入时间行使用 flex 对齐时钟图标和文字；当前学校内容改为内缩圆角卡片，修复贴住两侧边框的问题。
  - 当前学校、校园认证、推荐隐私统一使用图标底座；推荐隐私补齐标题行布局、分隔线和右侧控件对齐。
  - 验证：小程序 typecheck/format、个人中心 WXML/WXSS 编译通过，模拟器截图确认头部、当前学校和推荐隐私区域布局正常。
  - 任务报告：[AIwork/小程序个人中心视觉与头像入口优化_任务报告.md](AIwork/小程序个人中心视觉与头像入口优化_任务报告.md)

- [x] **MP-MAP-SHEET-FOLLOW** 修复地图地点抽屉跟手拖拽（2026-08-07）：
  - 将半屏抽屉的 `touchstart/touchmove/touchend` 绑定到实际承载内容的 `scroll-view`，避免父级 view 收不到连续移动事件。
  - 半屏时关闭内容滚动、实时更新整块抽屉的 `translate3d`；全屏时保留同一 `scroll-view` 的内容滚动。
  - 自动化触摸验证：移动 120px 时 `sheetDragOffset=-120`，抽屉布局 top 同步移动到约 240px；释放后按阈值进入全屏。
  - 任务报告：[AIwork/小程序地图抽屉跟手拖拽修复_任务报告.md](AIwork/小程序地图抽屉跟手拖拽修复_任务报告.md)

- [x] **MP-MAP-SHEET-COMPOSITOR-FIX** 修复地图抽屉真机拖动只吸附、不显示中间帧（2026-08-07）：
  - 移除抽屉入场动画对 `transform` 的占用，避免 `animation-fill-mode: both` 覆盖跟手拖拽写入的内联位移；入场动画仅保留透明度变化。
  - 为抽屉声明 `will-change: transform`，稳定真机合成层上的连续位移渲染。
  - 自动化中间帧验证：未松手时 `sheetDragOffset=-140`，抽屉顶部由 `240.453px` 同步移动到 `100.453px`；松手后再吸附至全屏 `4px`。
  - 任务报告：[AIwork/小程序地图抽屉真机跟手渲染修复_任务报告.md](AIwork/小程序地图抽屉真机跟手渲染修复_任务报告.md)

- [x] **MP-MAP-SHEET-COLLAPSE** 补齐地图全屏抽屉下拉缩回链路（2026-08-07）：
  - 全屏时持续记录统一内容区的滚动位置；仅在内容位于顶部且继续下拉时接管手势，整张抽屉跟手移动并按阈值缩回半屏。
  - 内容未到顶部时保留原生内容滚动；短距离下拉松手回弹全屏，避免误触缩回。
  - 自动化验证：全屏下拉 140px 时顶部由 `4px` 同步移动到 `144px`，松手后缩回半屏 `240.453px`；下拉 40px 松手回弹至 `4px`；模拟非零滚动位置时抽屉位移保持为 0。
  - 任务报告：[AIwork/小程序地图全屏抽屉下拉缩回_任务报告.md](AIwork/小程序地图全屏抽屉下拉缩回_任务报告.md)
